 ...
 *  passar um robô — e é por isso que bater nesta lista PONTUA em vez de
 *  descartar direto (ver `detectBot`). */
...
export function detectBot(signals: BotSignals): BotVerdict {
  ...
  /**
   * IP de datacenter de plataforma: sinal FORTE, mas não conclusivo.
   *
   * Já foi descarte direto aqui, e isso se mostrou errado na prática: o IP é o
   * dado menos confiável da cadeia — chega por `x-forwarded-for`, que qualquer
   * salto intermediário pode reescrever. Num ambiente com Docker Desktop na
   * frente, o endereço do par apresentado ao container caiu numa faixa do
   * Google, e o resultado foi TODO clique humano ser descartado como robô, em
   * silêncio, com a criadora vendo zero no analytics.
   *
   * 90 pontos, com limite em 100: sozinho não descarta um acesso que se
   * comporta como gente (user agent de navegador, idioma declarado, accept
   * específico). Somado a qualquer sinal fraco, descarta. É a mesma preferência
   * que o resto deste arquivo declara — falso positivo apaga clique humano do
   * relatório, o que é pior do que deixar passar um robô. Robô de plataforma
   * quase sempre também se identifica no user agent, onde o descarte continua
   * direto.
   */
  if (signals.ip && isDatacenterIP(signals.ip)) {
    add(90, "IP de datacenter de plataforma");
  }
